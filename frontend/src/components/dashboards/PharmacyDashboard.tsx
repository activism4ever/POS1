...
const PharmacyDashboard: React.FC = () => {
  ...
  const fetchPaidServices = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_BASE_URL}/pharmacy/paid-services`);
      setServices(response.data);
    } catch (err) {
      console.error('Failed to fetch paid pharmacy services:', err);
  ...
  const handleStartService = async (serviceId: number) => {
    ...
    try {
      await axios.put(`${API_BASE_URL}/pharmacy/start-service/${serviceId}`);
      setSuccess('Prescription preparation started');
      fetchPaidServices();
    } catch (err: any) {
  ...
  const handleCompleteService = async (serviceId: number) => {
    ...
    try {
      await axios.put(`${API_BASE_URL}/pharmacy/complete-service/${serviceId}`);
      setSuccess('Prescription dispensed and removed from queue');
      fetchPaidServices();
    } catch (err: any) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'paid':
        return <span className="badge bg-warning">Awaiting Dispensing</span>;
      case 'in_progress':
        return <span className="badge bg-info">Preparing</span>;
      case 'completed':
        return <span className="badge bg-success">Dispensed</span>;
      default:
    ...
    if (service.status === 'paid') {
      return (
        <>
          ...
          >
            {processing[service.id] ? 'Starting...' : 'Start Preparation'}
          </button>
          <button
            ...
            disabled={processing[service.id]}
          >
            {processing[service.id] ? 'Dispensing...' : 'Mark Dispensed'}
          </button>
        </>
      );
    ...
    return (
      <DashboardLayout title="Pharmacy Dashboard">
        <div className="text-center p-4">
          ...
          <p className="mt-2">Loading pharmacy services...</p>
        </div>
      </DashboardLayout>
    );
  }
            ...
            <div className="card-body">
              ...
              {services.length === 0 ? (
                <div className="text-center py-4">
                  ...
                  <p className="text-muted mt-2">No prescriptions in queue</p>
                </div>
              ) : (
